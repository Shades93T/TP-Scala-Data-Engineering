...
object TP_MR {
  ...
  def nthA[T](x: Int, l: List[T]): Option[T] = (x,l) match {
      case (0, h :: _) => Some(h)
      case (x, _ :: tail) if x > 0 => nthA(x-1, tail)
      case (_, Nil) => None
      case (_, h :: _) => None
  }
  ...
  //CLASSIC PATTERN MATCH ALGORITHM RECURSIVE
  def salarySum(employees: List[Employee]): Double = employees match {
      case h :: Nil => h.salary
      case h :: tail => (h.salary + salarySum(tail))
      case Nil => 0.0
  }
 ...
 // employees.foldLeft(0){(element, acc) => element.salary + acc}
  ...
  def addressOf(users: List[User]): List[String] = users match {
    case h :: Nil => h.address :: Nil
    case h :: tail => (h.address :: Nil) ::: addressOf(tail)
    case Nil => Nil
  }
// OR =============> users.map(user => user.address)
  ...
  def average(values: Iterator[Double]): Option[Double] = {
    if (values.isEmpty) { None }  
      ...
      val result: (Int, Double) = values.foldLeft((0, 0.0)) { case ((size, sum), element) => (size+1, sum + element) }
      Some(result._2 / result._1)
    }
  }
  ...
  implicit def tupleMonoid[A: Monoid, B: Monoid]: Monoid[(A, B)] =
    new Monoid[(A, B)] {
      ...
      override def combine(left: (A, B), right: (A, B)): (A, B) = (Monoid[A].combine(left._1, right._1),
